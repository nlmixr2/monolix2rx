...
rxSolve.monolix2rx <- function(object, params = NULL, events = NULL, 
    ...
    nsim = NULL, minSS = 10L, maxSS = 10000L, infSSstep = 12, 
    ...
    safePow = TRUE, sumType = c("pairwise", "fsum", "kahan", 
        ...
        "logify"), resample = NULL, resampleID = TRUE, maxwhile = 1e+05, 
    atolSens = 1e-08, rtolSens = 1e-06, ssAtolSens = 1e-08, ssRtolSens = 1e-06, 
    simVariability = NA, nLlikAlloc = NULL, useStdPow = FALSE, 
    naTimeHandle = c("ignore", "warn", "error"), addlKeepsCov = FALSE, 
    addlDropSs = TRUE, ssAtDoseTime = TRUE, ss2cancelAllPending = FALSE, 
    ssSolved = TRUE, linCmtSensType = c("auto", "endpoint5", 
        "endpoint5G", "forward3", "forward3G", "AD", "central", 
        "forward", "forwardG", "forwardH", "centralH", "forward3H", 
        "endpointH5", "forwardG"), linCmtSensH = 1e-04, linCmtGillFtol = 0, 
    linCmtGillK = 20L, linCmtGillStep = 4, linCmtGillRtol = sqrt(.Machine$double.eps), 
    linCmtShiErr = sqrt(.Machine$double.eps), linCmtShiMax = 20L, 
    linCmtScale = FALSE, linCmtHcmt = NULL, linCmtHmeanI = c("geometric", 
        "arithmetic", "harmonic"), linCmtHmeanO = c("geometric", 
        "arithmetic", "harmonic"), linCmtSuspect = 1e-06, linCmtForwardMax = 2L, 
    envir = parent.frame()) {
    if (missing(cores)) {
        cores <- 0L
    }
    ...
    rxode2::rxSolve(object = object, params = params, events = events, 
        ...
        ssSolved = ssSolved, linCmtSensType = linCmtSensType, 
        linCmtSensH = linCmtSensH, linCmtGillFtol = linCmtGillFtol, 
        linCmtGillK = linCmtGillK, linCmtGillStep = linCmtGillStep, 
        linCmtGillRtol = linCmtGillRtol, linCmtShiErr = linCmtShiErr, 
        linCmtShiMax = linCmtShiMax, linCmtScale = linCmtScale, 
        linCmtHcmt = linCmtHcmt, linCmtHmeanI = linCmtHmeanI, 
        linCmtHmeanO = linCmtHmeanO, linCmtSuspect = linCmtSuspect, 
        linCmtForwardMax = linCmtForwardMax, envir = envir)
}
